...
def pip_internal_location_path(pkg_name, input_prefix):
    # when prefix is given...let's borrow pip._internal to find the location ;D
    import pip._internal.locations
    return pip._internal.locations.get_scheme(
            pkg_name, prefix=input_prefix).purelib
...
def external_install_prefix(verbose=True):
    ...
    if '--user' in sys.argv:
        ...
        # when prefix is given...let's borrow pip._internal to find the location ;D
        path = pip_internal_location_path("mfem", prefix)
        if not os.path.exists(path):
            os.makedirs(path)
        path = os.path.join(path, 'mfem', 'external')
        print("no --user path:", path)
        return path
    '''
    else:
...
class InstallLib(_install_lib):
    def finalize_options(self):
        ...
        if '--user' in sys.argv:
            ...
            # when prefix is given...let's borrow pip._internal to find the location ;D
            self.install_dir = pip_internal_location_path("mfem", prefix)
        print("InstallLib install_dir: ", self.install_dir)
...
class InstallEggInfo(_install_egg_info):
    def finalize_options(self):
        ...
        if '--user' not in sys.argv:
            # when prefix is given...let's borrow pip._internal to find the location ;D
            self.install_dir = pip_internal_location_path("mfem", prefix)
        print("InstallEggInfo install_dir: ", self.install_dir)
