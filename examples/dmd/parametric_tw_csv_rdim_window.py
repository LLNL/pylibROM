...
#  *
#  *****************************************************************************
# // Compile with: make parametric_tw_csv_rdim_window
# //
...
if __name__ == "__main__":
    ...
    parser.add_argument("-rdim_window_file", "--rdim_window_filename", dest='RDIMWindowfileName',
                        action='store', default='window.csv', type=str,
    ...
    window_master_list=[]
    with open((list_dir) + "/" + RDIMWindowfileName, mode ='r') as file:
        csvFile = csv.reader(file)
        csv_lines_ctr=0 #counter for number of lines in csvFile
        for lines in csvFile:
            csv_lines_ctr+=1
            window_master_list.append(lines)
    if window_master_list[0][0]=='RDIM':
    ...
    if (offline):
        ...
        for idx_dataset in range(npar):
            ...
            assert(((csv_lines_ctr-1)*windowNumSamples) == tot_samples), "Mismatch between -nwinsamp, number of windows in -rdim_window_file, and total number of samples"

            for window in range(numWindows):
                if (useWindowDims):
                    rdim = windowDim[window]
                if len(window_master_list)>0:
                    if use_rdim_windows==True:
                        rdim=int(window_master_list[window+1][1])
                if (rdim != -1):
                    if (myid == 0):
